...
const schema: FieldType[] = [
  ...
  {
    name: "looking-for",
    typ: { cases: ["free", "premium", "vip"], type: "Enum" },
  },
  ...
  const generateJsonSchema = (field: FieldType): any => {
    ...
    if (typ.type === "Option") {
      return {
        anyOf: [
          { type: "null" },
          generateJsonSchema({ name: field.name, typ: typ.inner }),
        ],
      };
    }
      ...
      case "U64":
        return (
          <Input
            ...
            placeholder={name}
            onChange={(e) =>
              handleInputChange(`input-${index}`, e.target.value)
            }
          />
        );
      case "Bool":
        return (
          <Checkbox
            ...
            onCheckedChange={(checked) =>
              handleInputChange(`input-${index}`, checked)
            }
          />
        );
      case "Enum":
