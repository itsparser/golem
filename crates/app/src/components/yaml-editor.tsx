...
export function YamlEditor({value, onChange}: YamlEditorProps) {
    ...
    const createMarker = (error: yaml.YAMLException, monaco: Monaco, severity: "Error" | "Warning") => {
    ...
    return (
        <div className="relative h-[400px] border rounded-md overflow-hidden">
            ...
            {markers &&
                <div
                    className="text-sm text-destructive bg-destructive/10 p-3 rounded-md">{markers.map(x => x.message).join(" \n")}</div>}
        </div>
    )
}
